...
#
#
# See accompanying init.py for configration.
...
def ManualLoad():
	...
	for menutype_path in menutype: #this iterates through all the menutypes
		for froot, fdir, ffiles in os.walk(menutype_path):
			if not os.path.isfile(froot + ignore) and not os.path.isfile(froot + autoinstaller): #ignore if the 'ignore'-file is present in the directory, or if the 'autoinstaller'-file is present in the directory
				...
				for itemName in os.listdir(group):
					if not itemName.endswith('.png'):
						...
						if (itemExt == '.gizmo' or (itemExt == '.nk' and os.path.basename(os.path.dirname(fullpath)).lower() != 'toolsets')):
							ToDo_Menu += mt(MenuType) + ".addCommand('" + afterMenuTypeNoExtUnderscore + "'" + ", \"nuke.createNode('" + itemName + "')\"" + icon + ")" + '\n' #createNode for gizmo and nuke scripts
							ItemCount += 1
						elif (itemExt == '.py'):
	...
	for autorun in autorun_menu:
		for item in os.listdir(autorun):
			if os.path.isfile(autorun + '/' + item) and not os.path.isfile(autorun + '/' + ignore):
				ToDo_Menu += "nuke.load('" + autorun + item + "')" + '\n'


	if (fontFound > 0): #if fonts are found, overwrite project setting
		ToDo_Menu += "\nnuke.knobDefault(\"Root.free_type_font_path\", \"" + root + dir_fonts + "\")" + '\n'


	#the following string is for cache files to be able to do user logging, if enabled
	WriteActiveUserBlock = r"""
# The following block writes the current username to the 'user_activity'-folder in 'Required', as 'write_active_user' is enabled in NukeShared config.

import getpass

username = getpass.getuser()
filedir = '""" + root_req + r"""user_activity'
...
"""
	...
	ToDo_Menu += 'fontFound = ' + (str)(fontFound) + '\n'
	...
	if(menuID == initID and menuID > 0): #warn user about font conflicts
		fontCount = 0
		for i in xrange(initID + 1):
			if (u_fontFound[i] > 0):
				fontCount += 1
		if fontCount > 1:
			print (NukeSharedPrint + "Warning: Only one font repository can be loaded at a time! Skipping other font reositories for now.")
...
def GetStats():
	return (str(ItemCount) + " plugins, " + (str)(pyFound) + " init.py/menu.py, " + (str)(fontFound) + " font" + ('s' if fontFound != 1 else '') + ", " + (str)(viewerprocessFound) + " viewerprocess" + ('es' if viewerprocessFound != 1 else '') + "." if showstats else '') #the stats string to append at the end of the confirmation (if enabled)
...
if(cached):
	...
	ManualLoad()
	if showconfirmation:
		print(NukeSharedPrint + custommessage + GetStats()) #final confirmation on regular load